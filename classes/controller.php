...
abstract class controllerSub {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameSub::_()->getModule( $this->getCode() );
		$className = '';
		if(import($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameSub($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(import($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameSub($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(SUB_ADMIN),
				'remove' => array(SUB_ADMIN),
				'restore' => SUB_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - SUB_METHODS or SUB_USERLEVELS
	 */
