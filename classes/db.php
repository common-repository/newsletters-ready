...
class dbSub {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see SUB_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, SUB_DB_PREF, $wpdb->prefix. SUB_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampSub($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, SUB_DATE_DL)) return dateToTimestampSub($date);
        $arr = explode('-', $date);
        return dateToTimestampSub($arr[2]. SUB_DATE_DL. $arr[1]. SUB_DATE_DL. $arr[0]);
    }
