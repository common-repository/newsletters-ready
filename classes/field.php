...
class fieldSub {
	 ...
	 */
	public $adapt = array('htmlSub' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterSub::_($value, $this->adapt['dbFrom'], fieldAdapterSub::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlSub', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterSub::_($this, $this->adapt['html'], fieldAdapterSub::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameSub::_()->getModule('optionsSub')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select'));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsSub'] = $options;
				}
			}
			$htmlContent = htmlSub::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlSub::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterSub::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterSub::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsSub');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameSub::_()->getModule('optionsSub')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsSub'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlSub', $method)) {
			$output .= htmlSub::$method($name, $params);
			$output .= htmlSub::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
		...
		if ($this->name == "params") {
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterSub::_($id,'getExtraFieldOptions',fieldAdapterSub::STR);
				$output .= '</div>';
				...
				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions').':<br />';
					...
					$output .= __('width').':<br />';
					$output .= htmlSub::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height').':<br />';
					$output .= htmlSub::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterSub::_($this, $this->adapt['htmlParams'], fieldAdapterSub::STR);
			} else {
				...
				$output .= fieldAdapterSub::_($params,'getFieldAttributes',  fieldAdapterSub::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsSub::xmlAttrToStr($param, $element);
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameSub::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameSub::_()->getModule($module)->isExternal())
		   $config_xml = frameSub::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = SUB_MODULES_DIR.$module.DS.'mod.xml';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameSub::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsSub'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlSub::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
