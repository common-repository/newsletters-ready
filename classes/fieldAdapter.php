...
class fieldAdapterSub {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterSub', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsSub'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsSub::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsSub::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $brands = frameSub::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To'). '
            <div id="tax_address">
                <b>'. __('Address'). '</b><br />
                '. __('Destination'). ':'. htmlSub::checkboxlist('params[dest]', array('optionsSub' => $listOfDest)). '<br />
                '. __('Country'). ':'. htmlSub::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories'). '</b><br />
                '. (empty($cOptions) ? __('You have no categories') : htmlSub::checkboxlist('params[categories][]', array('optionsSub' => $cOptions))). '<br />
                    <b>'. __('Brands'). '</b><br />
                '. (empty($bOptions) ? __('You have no brands') : htmlSub::checkboxlist('params[brands][]', array('optionsSub' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate').': '. htmlSub::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute').': '. htmlSub::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameSub::_()->getModule('optionsSub')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlSub::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class').':<br />';
                $output .= htmlSub::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id').':<br />';
                $output .= htmlSub::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsSub']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsSub'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All');
            foreach($categories as $c) {
                $field->htmlParams['optionsSub'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameSub::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameSub::_()->getTable('states')
				->leftJoin( frameSub::_()->getTable('countries'), 'country_id' )
