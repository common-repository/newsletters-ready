...
class htmlSub {
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = SUB_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
        $out = '';
        if(strpos($params['url'], 'pl='. SUB_CODE) === false)
			$params['url'] = uriSub::_(array('baseUrl' => $params['url'], 'pl' => SUB_CODE));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterSub::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsSub::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterSub::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlSub::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsSub::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= __($p['label']). htmlSub::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadCategoriesOptions() {
        if(empty(self::$categoriesOptions)) {
            $categories = frameSub::_()->getModule('products')->getCategories();
            if(!empty($categories)) {
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameSub::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function colorpicker($name, $params = array('value' => '', 'id' => '', 'attrs' => '', 'change' => '')) {
		$params = empty($params) ? array() : $params;
		$params['id'] = isset($params['id']) && !empty($params['id']) ? $params['id'] : self::nameToClassId($name). '_'. mt_rand(9, 9999);
		$params['attrs'] = isset($params['attrs']) ? $params['attrs'] : '';
		$params['attrs'] .= ' id="'. $params['id']. '"';
		$colorPickerSettings = '';
		// We don't use here jsonEncode - as it will encode "change" param as a string, but it should be a function - for javascript usage, 
		// so not change: "myCallback", but change: myCallback, but jsonEncode will do it in first way
		if(isset($params['change']))
			$colorPickerSettings .= 'change:'. $params['change'];
		if(isset($params['value']))
			$colorPickerSettings .= (empty($colorPickerSettings) ? '' : ','). 'defaultColor:"'. $params['value']. '"';
		if(!empty($colorPickerSettings))
			$colorPickerSettings = '{'. $colorPickerSettings. '}';
		$out = self::text($name, $params);
		$out .= '<script type="text/javascript">//<!--
			jQuery(function(){
				jQuery("#'. $params['id']. '").wpColorPicker('. $colorPickerSettings. ');
			});
			//--></script>';
		return $out;
	}
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			foreach(fieldAdapterSub::getFontsList() as $font)
				$options[ $font ] = $font;
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		$params['id'] = isset($params['id']) && !empty($params['id']) ? $params['id'] : (self::nameToClassId($name). '_'. mt_rand(1, 99999));
		$params['checked'] = isset($params['checked']) ? (int) $params['checked'] : 0;
		$params['attrs'] = isset($params['attrs']) && !empty($params['attrs']) ? $params['attrs'] : '';
		$params['attrs'] .= ' id="'. $params['id']. '"';
		
		return '<a class="toeSlideShellSub" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtSub"></span>
			<span class="toeSlideOnSub">'. __('ON'). '</span>
			<span class="toeSlideOffSub">'. __('OFF'). '</span>
