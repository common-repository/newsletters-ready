...
abstract class tableSub {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(SUB_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjSub('fieldSub', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbSub::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbSub::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbSub::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothig to update'));
            return false;
        }
        ...
        if(dbSub::query($query)) {
            if($method == 'INSERT')
                return dbSub::lastID();
            else
                return true;
        } else {
			$this->_addError( mysql_error() );
		}
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if(isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterSub::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterSub::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorSub::_($objForValidation)) {
					...
					$type = isset($this->_fields[$k]) ? $this->_fields[$k]->type : 'default';
					switch($type) {
						case 'int':
						case 'tinyint':
							$res .= $k. ' = '. (int)$val. ' '. $delim. ' ';
    ...
    /**
     * Add new fieldSubSub for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjSub('fieldSub', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbSub::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(SUB_DATE_FORMAT, dbSub::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbSub::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
