...
class utilsSub {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsSub::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsSub::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
	...
	/**
	 * Get array with days of week, from 1 to 7 - from Monday to Sunday
	 * @return array - days
	 */
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsSub::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModDir($plugName) {
        if(strpos($plugName, self::getThemesSubDir()) !== false) {  //for modules in theme dir
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, self::getThemesSubDir()) !== false) {  //for modules in theme dir
            return SUB_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function getCurrentWPThemeCode() {
        ...
        if(empty($activeThemeName)) {
            ...
            $temp = explode(self::getThemesSubDir(). '/', $url);
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameSub::_()->getModule($m)) 
                return false;
            if(!is_dir(frameSub::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameSub::_()->getModules() as $m) {
	 ...
	 */
	static public function getCategories($args = array()) {
	...
	/**
	 * In usual wordpress setup should return wp-content/themes
	 */
	static public function getThemesSubDir() {
		return str_replace(ABSPATH, '', get_theme_root());
	}
}
