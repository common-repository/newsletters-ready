 ...
 */
if(!function_exists('dateToTimestampSub')) {
    function dateToTimestampSub($date) {
        if(empty($a))
            return false;
        $a = explode(SUB_DATE_DL, $date);
...
if(!function_exists('importClassSub')) {
    function importClassSub($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), SUB_CODE) !== false) {
					$classFile = preg_replace('/'. SUB_CODE. '/i', '', $classFile);
				}
                $path = SUB_CLASSES_DIR. $classFile. '.php';
			}
            return import($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, SUB_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], SUB_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameSub')) {
    function toeGetClassNameSub($class) {
        $className = '';
		if(class_exists($class. strFirstUp(SUB_CODE))) 
			$className = $class. strFirstUp(SUB_CODE);
		else if(class_exists(SUB_CLASS_PREFIX. $class)) 
            $className = SUB_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjSub')) {
    function toeCreateObjSub($class, $params) {
        $className = toeGetClassNameSub($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsSub')) {
	function prepareParamsSub(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsSub::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeSub($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeSub')) {
	function prepareFieldCodeSub($string) {   
...
if(!function_exists('toeDateSub')) {
	function toeDateSub($format, $timestamp = null) {
		if(is_null($timestamp))
			$timestamp = time();
		return get_date_from_gmt(date('Y-m-d H:i:s', $timestamp), $format);
	}
}
if(!function_exists('toeCurrentTimeSub')) {
	function toeCurrentTimeSub($type = 'timestamp', $gmt = 0) {
