...
<div class="container">
	<form id="toeWelcomePageFindUsForm">
		<h1>
			<?php _e('Welcome to')?>
			<?php echo SUB_WP_PLUGIN_NAME?>
			<?php _e('Version')?>
			<?php echo SUB_VERSION?>!
		</h1>
		<div class="clear"></div>
		<div class="about-message">
			This is first start up of the <?php echo SUB_WP_PLUGIN_NAME?> plugin.<br />
		...
		<div class="plug-icon-shell">
			...
			<a target="_blank" href="http://readyshoppingcart.com/"><?php echo SUB_WP_PLUGIN_NAME?></a><br />
			<a target="_blank" href="http://readyshoppingcart.com/"><?php echo SUB_VERSION?></a><br />
		</div>
		<div class="clear"></div>
		...
		<?php foreach($this->askOptions as $askId => $askOpt) { ?>
			<label><?php echo htmlSub::radiobutton('where_find_us', array('value' => $askId))?>&nbsp;<?php echo $askOpt['label']?></label>
			<?php if($askId == 4 /*Find on the web*/) { ?>
				<label id="toeFindUsUrlShell" style="display: none;">Please, post url: <?php echo htmlSub::text('find_on_web_url')?></label>
			<?php } elseif($askId == 5 /*Other way*/) { ?>
				<label style="display: none;" id="toeOtherWayTextShell"><?php echo htmlSub::textarea('other_way_desc')?></label>
			<?php }?>
		<?php }?>
		...
		<div class="col-3">
			<h3>Documentation:</h3>
			<p>Check <a target="_blank" href="http://readyshoppingcart.com/">documentation</a> and FAQ section. If you can't solve your problems - <a target="_blank" href="http://readyshoppingcart.com/contacts/">contact us</a>.</p>
		</div>
		...
		<?php echo htmlSub::hidden('page', array('value' => 'promo_ready'))?>
		<?php echo htmlSub::hidden('action', array('value' => 'welcomePageSaveInfo'))?>
		<?php echo htmlSub::hidden('reqType', array('value' => 'ajax'))?>
		<?php echo htmlSub::submit('gonext', array('value' => 'Thank for check info. Start using plugin.', 'attrs' => 'class="button button-primary button-hero"'))?>
		<?php echo htmlSub::hidden('original_page', array('value' => reqSub::getVar('page')))?>
