...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    */
	...
	function getAllChildren() {
		...
		if(!empty($children)) {
			foreach($children as $c) {
				$subChilds = $c->getAllChildren();
				if(!empty($subChilds)) {
					$children = array_merge($children, $subChilds);
				}
			}
		}
		return $children;
	...
	function getStyleArray() {
		...
		$styleStr = $this->style;
		if(!empty($styleStr)) {
			$styleParts = array_map('trim', explode(';', $styleStr));
			foreach($styleParts as $part) {
				$styleKeyVal = array_map('trim', explode(':', $part));
				if(!empty($styleKeyVal[0]))
					$styleArr[ $styleKeyVal[0] ] = $styleKeyVal[1];
			}
		}
		return $styleArr;
	}
	function setStyleFromArray($newStyleArr) {
		$newStyleParts = array();
		foreach($newStyleArr as $key => $val) {
			$newStyleParts[] = $key. ':'. $val;
		}
		$this->style = implode(';', $newStyleParts);
	}
	function css($style = array()) {
		$currStyles = $this->getStyleArray();
		foreach($style as $key => $val) {
			$currStyles[ $key ] = $val;
		}
		$this->setStyleFromArray( $currStyles );
	}
	/*****/
...
class simple_html_dom
{
    ...
    {
		/*UKRAINECMK*/
		// For case when we try to use * selector - let's return all child elements
		if($selector === '*' && is_null($idx)) {
